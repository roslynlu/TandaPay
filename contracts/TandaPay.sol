...
contract Tandapay {
    ...
    function startActivePeriod(uint groupId) public {
      require(msg.sender == groups[groupId].secretary); //Only secretary can start active period
      require(groups[groupId].paidPremiumCount == groups[groupId].userList.length); //All premiums have been paid
      require(groups[groupId].newPeriod.phase != Phase.ACTIVE); //Group is not active
      groups[groupId].newPeriod.phase = Phase.ACTIVE;
    }
